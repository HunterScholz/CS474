...
{
    int dir_index = (vaddr >> 16) & 0b11111111;
    int pt_index = (vaddr >> 8) & 0b11111111;
    int offset = vaddr & 0b11111111;

    int pt_entry = get_page_table_entry(dir_page_num, dir_index);
    if(pt_entry == 0){
        return 0;
    }

    int ppage = get_page_table_entry(pt_entry, pt_index);
    if(ppage == 0){
        return 0;
    }

    return get_page_start_addr(ppage) + offset;
}
