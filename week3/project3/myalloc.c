...
// Create a new block
struct block *block_new(int size, int in_use, struct block *current){
    struct block *b = PTR_OFFSET(current, size + padded_struct_block_size);
...
// Finds a free space and splits
void *find_space_and_split(struct block *head, int new_size){
    struct block *current = head;

    while(current != NULL){
        // First Fit
        if(current->size >= new_size && !current->in_use){
            // Turn current into used block
            int split_size = current->size - new_size;
            current->size = new_size;
            current->in_use = 1;

            // Create new unused block for split
            if(split_size > 0){
                struct block *n = block_new(split_size - padded_struct_block_size, 0, current);
                n->next = current->next;
                current->next = n;
            }
...
void *myalloc(int size){
    ...
    if(head == NULL){
        ...
        int data_size = 1024 - padded_struct_block_size;
        head->size = data_size;
        head->in_use = 0;
        head->next = NULL;
    }

    struct block *current = find_space_and_split(head, actual_size);
 ...
 */
void myfree(void *p){
    struct block *b = PTR_OFFSET(p, -padded_struct_block_size);
    b->in_use = 0;
    ...
    while(current != NULL && current->next != NULL){
        ...
        if(!current->in_use && !next_node->in_use){
            current->size += padded_struct_block_size + next_node->size;
